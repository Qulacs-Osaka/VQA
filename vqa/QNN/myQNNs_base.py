...
class myQNN_base(ABC):
    ...
    def compile(self, optimizer='SGD', loss='cross_entropy', hpara=None, init_params=None, min_approx_shots=None,
    ...
    def fit(self, init_params=None, options=None, hpara=None, init_spec_dict=None, Return_results=True, store_results_in_self=False,
            auto_save=True, save_self_obj=True, save_directory='single_optimization', process_id=0, **kwargs):
        '''
    ...
    def _eval_state_exact_expect_default(self, state):
        '''
    ...
    def _eval_state_exact_expect_var_singlePauli(self, state):
        ...
        '''
        exp_val = self._eval_state_exact_expect_default(state)
        var = 1. - exp_val**2
        return exp_val, var
    ...
    def _sample_outcomes_Z_single_data(self, x, n_shots):
        '''
        ...
        return self._sample_state_Z(state, n_shots, Return_outcomes=True)
    
    def _sample_outcomes_sum_Z_all_data(self, params, x_eval, n_s_list):
        '''
        ...
        for i in range(len(x_eval)):
            outcomes = self._sample_outcomes_Z_single_data(x_eval[i], n_s_list[i])
    ...
    # def _sample_outcomes_Z_all_data(self, params, x_eval, n_s_list, n_tot,,order=2):
    #     '''
    #     n_s_list[i] i-th dataに使うショット数のリスト（0でないものだけを取り出したもの推奨）
    #     x_eval のdata点のインデックスは、n_s_listのindexと同期している必要がある。
    #     n_tot: 分配元の、total number of shots. n_s_listが2倍されているなら、n_totも２倍。
    ...
    def _single_state_hybrid_sampler(self, state, n_shots, min_approx_shots, Return_var=False):
        ...
        if n_shots < min_approx_shots or min_approx_shots < 0:
            return self._sample_state_Z(state, n_shots, Return_var=Return_var)
        else:
            #CLTで近似
    ...
    def _single_state_hybrid_sampler_power(self, state, n_shots, min_approx_shots, order=4, Return_var=False, unbiased=False):
        ...
        if n_shots < min_approx_shots or min_approx_shots < 0:
            if Return_var:
                exp_val, sample_var = self._sample_state_Z(state, n_shots, Return_var=Return_var)
                EO_list = self._unbiased_powerE_Pauli(exp_val, n_shots, order=order, unbiased=unbiased)
                return EO_list, sample_var
            else:
                exp_val = self._sample_state_Z(state, n_shots, Return_var=Return_var)
                EO_list = self._unbiased_powerE_Pauli(exp_val, n_shots, order=order, unbiased=unbiased)
                return EO_list
        else:
            #CLTで近似
    ...
    def _exact_eval_obs_single_data(self, x):
        ...
        return self._eval_state_exact_expect_default(state)
    ...
    def sample_predict(self, x_eval, n_s_list, params=None, min_approx=None, Return_var=False):
        '''
        _sample_predict_scaled または _sample_predict_no_scale を選ぶなどして定義する。
        return super()._sample_predict_scaled(x_eval, n_s_list, params, min_approx, Return_var)
        '''
        pass

    def _sample_predict_scaled(self, x_eval, n_s_list, params=None, min_approx=None, Return_var=False):
        '''
        regressionなどでscaling parameterをparams[-1]に入れている場合に使う
    ...
    def _sample_predict_no_scale(self, x_eval, n_s_list, params=None, min_approx=None, Return_var=False):
        '''
        regressionなどでscaling parameterをparams[-1]に入れている場合に使う
        データ列 x_eval (list)の各データすべてについて n_s_list の要素のショット数を使って、observableの測定をし、
        params[-1]でスケーリングして最終的なpredictのリストを返す。
    ...
    def _direct_sample_MSEgrad_conditional_varUB_single_data(self, outcomes_no_shift, outcomes_PSR=None, Der_of_scale=False):
        ...
        self._sample_Z_single_data で返したoutcomesのリストを渡す。
...
#             logger.debug(f'est circ para: {est}')
    ...
    def _estimate_MSEgrad_mean_condVar(self, y_eval, E0pow_data_list, w=1., second_order=True, n0=None,
                                       Scale_para=True, Eppow_data_list=None, Empow_data_list=None,
                                      var_0=None, var_p=None, var_m=None):
        '''
        解析式をもとに、個別の推定量から E_data[ V[grad[i, data]|data] ]の不偏推定を計算して返す。
        （1/n_shots の係数として使うと、実際の勾配推定量の分散のupper boundとなる）
        Scale_para=True なら scale parameterの勾配の分散を計算
...
#                logger.debug(f'biased EV: {mean_condVar} \n \n')
    ...
    def MSE_grad_eval(self, params, n_shots_list, mini_batch_size=None, option_min_approx=None, option_weight=None):
    ...
    def MSE_iEval(self, params, n_shots_list, option_min_approx=None, option_weight=None, mini_batch_size=None, Max_n_shots0=True, second_order=True):
        '''
...
#         n = n_shots_list[-1]
#         outcomes0_data_list, outsum0 = self._sample_outcomes_sum_Z_all_data(params, x_eval, n_shots_data_list)
#         gEV = 0.
#         b = 2*w
...
#             outcomes_p_data_list, outsum_p = self._sample_outcomes_sum_Z_all_data(params + ei, x_eval, n_shots_data_list)
#             outcomes_m_data_list, outsum_m = self._sample_outcomes_sum_Z_all_data(params - ei, x_eval, n_shots_data_list)
#             gEV = 0.
...
#             gvar_list_b[i] = np.var(grad_data_list[i], ddof=1)
    ...
    def MSE_iEval_iRandom(self, params, n_shots_list, option_weight=None):
        '''
        ...
        for i in range(len(params)):
            ...
            if i == (len(params) - 1): #scale parameter w の勾配
                ...
                outcomes0_data_list, outsum = self._sample_outcomes_sum_Z_all_data(params, x_eval, n_shots_eval)
...
#                else:
                ...
                outcomes0_data_list, outsum0 = self._sample_outcomes_sum_Z_all_data(params, x_eval, n_shots_eval)
                ei = np.zeros_like(params)
                ei[i] = 0.5 * np.pi
                outcomes_p_data_list, outsum_p = self._sample_outcomes_sum_Z_all_data(params + ei, x_eval, n_shots_eval)
                outcomes_m_data_list, outsum_m = self._sample_outcomes_sum_Z_all_data(params - ei, x_eval, n_shots_eval)
    ...
    def grid_search_kCV(cls, optimizer_name, circuit, n_qubits, x_data, y_data, grid_para, fixed_para=None, loss='mean_squared_error', options=None,
                        ...
                        init_spec_dict=None, manual_methods_setting_dict=None, score_evaluator=None, random_seed_data = None,
                        shot_time=1.0, per_circuit_overhead=0.0, communication_overhead=0.0, return_all_results=True, **kwargs):
        '''
        ...
        if n_splits == 1:
            ...
            kf = KFold(n_splits=n_splits, random_state=random_seed)
...
#             print(f"Date format: {handler.formatter.datefmt}")
        def single_proc(args, idx):
            ...
            results = qnn.fit(options=options, auto_save=True, process_id=args, **kwargs)
            if return_all_results:
                return results, idx
            else:
    ...
    def compare_optimizers_kCV(cls, optimizers, circuit, n_qubits, x_data, y_data, loss='mean_squared_error', n_splits=5, repeat_kCV=True,
                        ...
                        parallel=False, processes=-1, min_approx_shots=-1, n_params=None, random_seed_data=None,                      
                        manual_methods_setting_dict=None, save_file_name=None, save_directory=None,
        ...
        #各試行のデータを準備する。
        opt_trials.bulk_add_trials(optimizers, train_test_inds, num_trials_per_combination, init_params, hpara_dict, optimization_options, compile_options, fit_kwargs_dict)
        idstr = str(uuid.uuid4())
                ...
                for trial_num in range(num_trials_per_combination):
                    ...
                    if init_params is None:
                        if (trial_num, split_id) not in init_params_cache:
                            init_para = np.random.uniform(0, 2*np.pi, n_params)
                            init_para[-1] = 1.
...
class OptimizationResultsAnalyzer:
    ...
    def cv_all_trials(self, x_label='total_shots', y_labels=None, stat_proc="mean_and_median"):
        '''
    ...
    def plot_statistics(self, data_to_plot=['cv_mean', 'cv_median'], x_label_name=None, y_label_name='', graph_title='', include_title=False,
                        save_figure=False, directory=None, file_extension='svg', xlim=None, ylim=None, xlog=False, ylog=False):
        '''
